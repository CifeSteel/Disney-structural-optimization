...
connectionCatalog = pd.read_csv('connection_catalog.txt', encoding='utf-16')
membCS = membCS.dropna()

membList = membCS.index.values
nodeList = dataNode.index.values

csProp = pd.read_csv('US_Section_Project_Square_only.txt', encoding='utf-16').set_index('AISC_Manual_Label [metric]')
		...
		for i in range(0,len(BrID)):
			if(BrID[i] != BrID[larBrInd] and BrID[i] != ''):
				if (i/4 == larBrInd/4):
					...
					angle = (90 if (BrDir[i] == BrDir[larBeInd]) else 0)
					# Assigning connection
					BrCS = membCS.at[BrID[i],'cross_section']
...
for i in range(0,len(connectionCatalog)):
	...
	dictKey = primary + secondary + str(angle)
	connectionDictID[dictKey] = conID
