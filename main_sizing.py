...
def read_section_data():
    ...
    sec_info_W=pd.DataFrame({'AISC_W_Data [metric]':sec_info_W["AISC_Manual_Label"],'FW_s':F_web,                              'FF_s':F_flange,'CW_s':C_web,'CF_s':C_flange,                             'tw':tw,'tf':tf,'rx':rx, "ry":ry, 'd':d,'b_t':b_t,'bf':bf,'h_tw':h_tw,                             'Sx':Sx,'Sy':Sy,'J':J,'rts':rts,'Ix':Ix,'Iy':Iy,'Zx':Zx,'Zy':Zy,'A':A,'W':sec_info_W.W,                             'unit_price':sec_info_W.unit_price})
    ...
    sec_info_HSS=pd.DataFrame({'AISC_HSS_Data [metric]':sec_info_HSS["AISC_Manual_Label"],'F_s':F_slenderness,                               'C_s':C_slenderness,'r':r, 'd_t':d_t,'D':D,'I':I,'S':S,'Z':Z,'A':A,'W':sec_info_HSS.W,                               'unit_price':sec_info_HSS.unit_price})
    ...
    if mem_id in graph_parents: # if mem_id has parents
        for pa_id in graph_parents[mem_id]: # for each parent
            if pa_id in active_member_list:
                ...
                if (sec_info['W'].d[upper_bound_cs]>sec_info['W'].d[pa_cs] - 2*sec_info['W'].tf[pa_cs]                     or sec_info['W'].bf[upper_bound_cs]>info_['W'].bf[pa_cs]): # find the smallest width of the parents' cross section
...
def calculate_load_per_unit_cost():
   ...
   #         sec_type="W"
        ...
        if mem_info.group[i] == 3:
            p1=mem_info.P[i]/sec_info["HSS"].A[size]+(mem_info.Mx[i]*sec_info["HSS"].D[size]/sec_info["HSS"].I[size]/2                                                +mem_info.My[i]*sec_info["HSS"].D[size]/sec_info["HSS"].I[size]/2)
            p2=mem_info.Mx[i]/mem_info.Mcx[i]+mem_info.My[i]/mem_info.Mcy[i]
            lpuc = ((mem_info.P[i]/sec_info["HSS"].A[size]) + (sec_info["HSS"].D[size]/(sec_info["HSS"].I[size]*2.0))                 * (mem_info.My[i] + mem_info.Mx[i])) * sec_info["HSS"].A[size]/1e20
        else:
